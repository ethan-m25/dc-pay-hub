#!/usr/bin/env python3
"""
dc-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Washington DC edition.
...
Run: python3 ~/dc-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/dc-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/dc-pay-hub/scripts/.greenhouse.lock")
...
SEED_SLUGS = load_greenhouse_seeds('dc')
...
def is_dc_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_dc_job(title, location, content_text):
            continue
...
def main():
    ...
    log("=== DC Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new DC jobs written to {OUTPUT_FILE} ===")
