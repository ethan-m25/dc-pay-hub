#!/usr/bin/env python3
"""
dc-pay-hub/scripts/_common.py
Shared utilities for all Washington DC Pay Hub search scripts.
DC Wage Transparency Act of 2024 (D.C. Law 25-194) — effective Jun 30, 2024 — all employers must include min/max projected salary + health care benefits description
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"dc-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/dc-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "DC"
...
REGION_TERMS = ['washington, dc', 'washington, d.c.', 'washington dc', 'district of columbia', 'washington d.c.', 'capitol hill', 'dupont circle', 'georgetown, dc', 'adams morgan', 'foggy bottom', 'navy yard', ', dc', 'dc,', 'd.c.']

_NON_REGION_LOC_TERMS = ['washington state', 'seattle, wa', 'bellevue, wa', 'redmond, wa', 'tacoma, wa', 'spokane, wa', 'kirkland, wa', 'seattle, washington', 'washington, wa', 'toronto', 'vancouver', 'ontario, canada', 'british columbia', 'canada,', 'denver, co', 'boulder, co', 'new york city', 'new york, ny', 'manhattan, ny', 'san francisco, ca', 'los angeles, ca', 'chicago, il', 'boston, ma', 'baltimore, md', 'newark, nj', 'minneapolis, mn', 'burlington, vt']
...
EXTRACT_PROMPT = """\
Extract ONE Washington DC job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Washington DC job with explicit USD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Washington DC location","source_url":"{url}","posted":"YYYY-MM-DD"}
...
- location must be in Washington DC
