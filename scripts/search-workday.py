#!/usr/bin/env python3
"""
dc-pay-hub/scripts/search-workday.py
Workday API scraper — Washington DC edition.

DC Wage Transparency Act (effective Jun 30, 2024)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Washington DC employers
  2. CXS JSON API   — paginate all jobs per employer, filter Washington DC (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/dc-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/dc-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/dc-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Washington DC"
...
# ── Seed tenants ─────────────────────────────────────────────────────────────
SEED_TENANTS = [
    ("bah.wd1.myworkdayjobs.com", "bah", "BAH_Jobs", "Booz Allen Hamilton"),
    ("saic.wd5.myworkdayjobs.com", "saic", "External", "SAIC"),
    ("leidos.wd3.myworkdayjobs.com", "leidos", "External", "Leidos"),
    ("gd.wd1.myworkdayjobs.com", "gd", "External", "General Dynamics"),
    ("northropgrumman.wd1.myworkdayjobs.com", "northropgrumman", "NGCCareers", "Northrop Grumman"),
    ("lockheedmartin.wd5.myworkdayjobs.com", "lockheedmartin", "External", "Lockheed Martin"),
    ("boeing.wd1.myworkdayjobs.com", "boeing", "EXTERNAL_CAREERS", "Boeing"),
    ("marriott.wd5.myworkdayjobs.com", "marriott", "External", "Marriott International"),
    ("capitalone.wd12.myworkdayjobs.com", "capitalone", "Capital_One", "Capital One"),
    ("danaher.wd1.myworkdayjobs.com", "danaher", "Careers", "Danaher"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Washington, DC" OR "Washington DC" salary 2026',
    'site:myworkdayjobs.com "Arlington" OR "Alexandria" Virginia salary 2026',
    'site:myworkdayjobs.com "Bethesda" OR "McLean" OR "Tysons" salary 2026',
    'site:myworkdayjobs.com DC defense OR consulting OR government salary 2026',
    'site:myworkdayjobs.com "Reston" OR "Herndon" Virginia tech salary 2026',
]
...
def is_dc(locations_text, external_path=""):
    """Return True only if the job is plausibly located in DC.
...
_BRAND_MAP = [
    ...
    (re.compile(r'booz allen hamilton', re.IGNORECASE),     "Booz Allen Hamilton"),
]
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_dc(p.get("locationsText", ""), p.get("externalPath", "")):
                    dc_jobs.append(p)
            offset += limit
        ...
        log(f"  DC jobs: {len(dc_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(dc_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_dc(location, external_path):
            continue
